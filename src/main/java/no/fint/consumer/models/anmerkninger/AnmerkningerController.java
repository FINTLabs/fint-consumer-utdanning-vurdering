...
import no.fint.model.resource.utdanning.vurdering.AnmerkningerResources;
import no.fint.model.utdanning.vurdering.VurderingActions;

@Slf4j
@Api(tags = {"Anmerkninger"})
@CrossOrigin
@RestController
@RequestMapping(name = "Anmerkninger", value = RestEndpoints.ANMERKNINGER, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class AnmerkningerController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private AnmerkningerCacheService cacheService;
    ...
    @GetMapping
    public AnmerkningerResources getAnmerkninger(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getAnmerkningerByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Anmerkninger cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, VurderingActions.GET_ALL_ANMERKNINGER, client);
        ...
        Stream<AnmerkningerResource> resources;
    ...
    @PostMapping("/$query")
    public AnmerkningerResources getAnmerkningerByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getAnmerkninger(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private AnmerkningerResources getAnmerkningerByOdataFilter(
        ...
        Event event = new Event(
                ...
                VurderingActions.GET_ANMERKNINGER, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new AnmerkningerResources();
    
        ArrayList<AnmerkningerResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<AnmerkningerResource>>() {});
    ...
    public AnmerkningerResource getAnmerkningerBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, VurderingActions.GET_ANMERKNINGER, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<AnmerkningerResource> anmerkninger = cacheService.getAnmerkningerBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return anmerkninger.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            AnmerkningerResource anmerkninger = objectMapper.convertValue(response.getData().get(0), AnmerkningerResource.class);
