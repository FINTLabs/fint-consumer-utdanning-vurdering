...
public class KarakterverdiController {
    ...
    public KarakterverdiResources getKarakterverdi(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getKarakterverdiByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Karakterverdi cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
    ...
    @PostMapping("/$query")
    public KarakterverdiResources getKarakterverdiByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getKarakterverdi(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private KarakterverdiResources getKarakterverdiByOdataFilter(
        ...
        Event event = new Event(
                ...
                VurderingActions.GET_KARAKTERVERDI, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new KarakterverdiResources();
    
        ArrayList<KarakterverdiResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<KarakterverdiResource>>() {});
    ...
    public KarakterverdiResource getKarakterverdiBySystemId(
        ...
        if (cacheService != null) {
            ...
            return linker.mapAndResetLinks(karakterverdi);
        }    
    }
