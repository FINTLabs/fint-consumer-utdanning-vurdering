...
public class UnderveisfagvurderingController {
    ...
    public UnderveisfagvurderingResources getUnderveisfagvurdering(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getUnderveisfagvurderingByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Underveisfagvurdering cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
    ...
    @PostMapping("/$query")
    public UnderveisfagvurderingResources getUnderveisfagvurderingByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getUnderveisfagvurdering(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private UnderveisfagvurderingResources getUnderveisfagvurderingByOdataFilter(
        ...
        Event event = new Event(
                ...
                VurderingActions.GET_UNDERVEISFAGVURDERING, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new UnderveisfagvurderingResources();
    
        ArrayList<UnderveisfagvurderingResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<UnderveisfagvurderingResource>>() {});
