...
import no.fint.model.resource.utdanning.vurdering.ElevvurderingResources;
import no.fint.model.utdanning.vurdering.VurderingActions;

@Slf4j
@Api(tags = {"Elevvurdering"})
@CrossOrigin
@RestController
@RequestMapping(name = "Elevvurdering", value = RestEndpoints.ELEVVURDERING, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class ElevvurderingController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private ElevvurderingCacheService cacheService;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Elevvurdering cache is disabled.");
        }
    ...
    @GetMapping
    public ElevvurderingResources getElevvurdering(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getElevvurderingByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Elevvurdering cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, VurderingActions.GET_ALL_ELEVVURDERING, client);
        ...
        Stream<ElevvurderingResource> resources;
    ...
    @PostMapping("/$query")
    public ElevvurderingResources getElevvurderingByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getElevvurdering(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private ElevvurderingResources getElevvurderingByOdataFilter(
        ...
        Event event = new Event(
                ...
                VurderingActions.GET_ELEVVURDERING, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new ElevvurderingResources();
    
        ArrayList<ElevvurderingResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<ElevvurderingResource>>() {});
    ...
    public ElevvurderingResource getElevvurderingBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, VurderingActions.GET_ELEVVURDERING, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<ElevvurderingResource> elevvurdering = cacheService.getElevvurderingBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return elevvurdering.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            ElevvurderingResource elevvurdering = objectMapper.convertValue(response.getData().get(0), ElevvurderingResource.class);
