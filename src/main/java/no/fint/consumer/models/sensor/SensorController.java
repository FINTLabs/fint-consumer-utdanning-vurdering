...
import no.fint.model.resource.utdanning.vurdering.SensorResources;
...
@CrossOrigin
@RestController
@RequestMapping(name = "Sensor", value = RestEndpoints.SENSOR, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class SensorController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private SensorCacheService cacheService;

    @Autowired
    private FintAuditService fintAuditService;

    @Autowired
    private SensorLinker linker;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Sensor cache is disabled.");
        }
    ...
    @GetMapping
    public SensorResources getSensor(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getSensorByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Sensor cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, VurderingActions.GET_ALL_SENSOR, client);
        ...
        Stream<SensorResource> resources;
    ...
    @PostMapping("/$query")
    public SensorResources getSensorByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getSensor(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private SensorResources getSensorByOdataFilter(
        ...
        Event event = new Event(
                ...
                VurderingActions.GET_SENSOR, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new SensorResources();
    
        ArrayList<SensorResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<SensorResource>>() {});
    ...
    public SensorResource getSensorBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, VurderingActions.GET_SENSOR, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<SensorResource> sensor = cacheService.getSensorBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return sensor.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            SensorResource sensor = objectMapper.convertValue(response.getData().get(0), SensorResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(sensor);
        }    
    }
