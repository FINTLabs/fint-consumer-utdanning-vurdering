...
@CrossOrigin
@RestController
@RequestMapping(name = "Karakterhistorie", value = RestEndpoints.KARAKTERHISTORIE, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class KarakterhistorieController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private KarakterhistorieCacheService cacheService;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Karakterhistorie cache is disabled.");
        }
    ...
    @GetMapping
    public KarakterhistorieResources getKarakterhistorie(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getKarakterhistorieByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Karakterhistorie cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, VurderingActions.GET_ALL_KARAKTERHISTORIE, client);
        ...
        Stream<KarakterhistorieResource> resources;
    ...
    @PostMapping("/$query")
    public KarakterhistorieResources getKarakterhistorieByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getKarakterhistorie(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private KarakterhistorieResources getKarakterhistorieByOdataFilter(
        ...
        Event event = new Event(
                ...
                VurderingActions.GET_KARAKTERHISTORIE, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new KarakterhistorieResources();
    
        ArrayList<KarakterhistorieResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<KarakterhistorieResource>>() {});
    ...
    public KarakterhistorieResource getKarakterhistorieBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, VurderingActions.GET_KARAKTERHISTORIE, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<KarakterhistorieResource> karakterhistorie = cacheService.getKarakterhistorieBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return karakterhistorie.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            KarakterhistorieResource karakterhistorie = objectMapper.convertValue(response.getData().get(0), KarakterhistorieResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(karakterhistorie);
        }    
    }
