package no.novari.fint.consumer.models.elevfravar;
...
@CrossOrigin
@RestController
@RequestMapping(name = "Elevfravar", value = RestEndpoints.ELEVFRAVAR, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class ElevfravarController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private ElevfravarCacheService cacheService;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Elevfravar cache is disabled.");
        }
    ...
    @GetMapping
    public ElevfravarResources getElevfravar(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getElevfravarByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Elevfravar cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, VurderingActions.GET_ALL_ELEVFRAVAR, client);
        ...
        Stream<ElevfravarResource> resources;
    ...
    @PostMapping("/$query")
    public ElevfravarResources getElevfravarByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getElevfravar(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private ElevfravarResources getElevfravarByOdataFilter(
        ...
        Event event = new Event(
                ...
                VurderingActions.GET_ELEVFRAVAR, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new ElevfravarResources();
    
        ArrayList<ElevfravarResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<ElevfravarResource>>() {});
    ...
    public ElevfravarResource getElevfravarBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, VurderingActions.GET_ELEVFRAVAR, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<ElevfravarResource> elevfravar = cacheService.getElevfravarBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return elevfravar.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            ElevfravarResource elevfravar = objectMapper.convertValue(response.getData().get(0), ElevfravarResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(elevfravar);
        }    
    }
