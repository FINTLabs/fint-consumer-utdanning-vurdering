package no.novari.fint.consumer.models.fravarsoversikt;
...
public class FravarsoversiktController {
    ...
    private FravarsoversiktCacheService cacheService;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Fravarsoversikt cache is disabled.");
        }
    ...
    @GetMapping
    public FravarsoversiktResources getFravarsoversikt(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getFravarsoversiktByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Fravarsoversikt cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, VurderingActions.GET_ALL_FRAVARSOVERSIKT, client);
    ...
    @PostMapping("/$query")
    public FravarsoversiktResources getFravarsoversiktByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getFravarsoversikt(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private FravarsoversiktResources getFravarsoversiktByOdataFilter(
        ...
        Event event = new Event(
                ...
                VurderingActions.GET_FRAVARSOVERSIKT, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new FravarsoversiktResources();
    
        ArrayList<FravarsoversiktResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<FravarsoversiktResource>>() {});
    ...
    public FravarsoversiktResource getFravarsoversiktBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, VurderingActions.GET_FRAVARSOVERSIKT, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<FravarsoversiktResource> fravarsoversikt = cacheService.getFravarsoversiktBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return fravarsoversikt.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            FravarsoversiktResource fravarsoversikt = objectMapper.convertValue(response.getData().get(0), FravarsoversiktResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(fravarsoversikt);
        }    
    }
