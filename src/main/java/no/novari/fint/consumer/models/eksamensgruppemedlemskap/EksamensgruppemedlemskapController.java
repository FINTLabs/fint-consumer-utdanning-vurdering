...
@Slf4j
@Api(tags = {"Eksamensgruppemedlemskap"})
@CrossOrigin
@RestController
@RequestMapping(name = "Eksamensgruppemedlemskap", value = RestEndpoints.EKSAMENSGRUPPEMEDLEMSKAP, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class EksamensgruppemedlemskapController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private EksamensgruppemedlemskapCacheService cacheService;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Eksamensgruppemedlemskap cache is disabled.");
        }
    ...
    @GetMapping
    public EksamensgruppemedlemskapResources getEksamensgruppemedlemskap(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getEksamensgruppemedlemskapByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Eksamensgruppemedlemskap cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, VurderingActions.GET_ALL_EKSAMENSGRUPPEMEDLEMSKAP, client);
        ...
        Stream<EksamensgruppemedlemskapResource> resources;
    ...
    @PostMapping("/$query")
    public EksamensgruppemedlemskapResources getEksamensgruppemedlemskapByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getEksamensgruppemedlemskap(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private EksamensgruppemedlemskapResources getEksamensgruppemedlemskapByOdataFilter(
        ...
        Event event = new Event(
                ...
                VurderingActions.GET_EKSAMENSGRUPPEMEDLEMSKAP, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new EksamensgruppemedlemskapResources();
    
        ArrayList<EksamensgruppemedlemskapResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<EksamensgruppemedlemskapResource>>() {});
    ...
    public EksamensgruppemedlemskapResource getEksamensgruppemedlemskapBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, VurderingActions.GET_EKSAMENSGRUPPEMEDLEMSKAP, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<EksamensgruppemedlemskapResource> eksamensgruppemedlemskap = cacheService.getEksamensgruppemedlemskapBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return eksamensgruppemedlemskap.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            EksamensgruppemedlemskapResource eksamensgruppemedlemskap = objectMapper.convertValue(response.getData().get(0), EksamensgruppemedlemskapResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(eksamensgruppemedlemskap);
        }    
    }
