package no.novari.fint.consumer.models.sluttfagvurdering;
...
import no.novari.fint.model.resource.utdanning.vurdering.SluttfagvurderingResources;
import no.novari.fint.model.utdanning.vurdering.VurderingActions;

@Slf4j
@Api(tags = {"Sluttfagvurdering"})
@CrossOrigin
@RestController
@RequestMapping(name = "Sluttfagvurdering", value = RestEndpoints.SLUTTFAGVURDERING, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class SluttfagvurderingController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private SluttfagvurderingCacheService cacheService;

    @Autowired
    private FintAuditService fintAuditService;

    @Autowired
    private SluttfagvurderingLinker linker;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Sluttfagvurdering cache is disabled.");
        }
    ...
    @GetMapping
    public SluttfagvurderingResources getSluttfagvurdering(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getSluttfagvurderingByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Sluttfagvurdering cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, VurderingActions.GET_ALL_SLUTTFAGVURDERING, client);
        ...
        Stream<SluttfagvurderingResource> resources;
    ...
    @PostMapping("/$query")
    public SluttfagvurderingResources getSluttfagvurderingByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getSluttfagvurdering(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private SluttfagvurderingResources getSluttfagvurderingByOdataFilter(
        ...
        Event event = new Event(
                ...
                VurderingActions.GET_SLUTTFAGVURDERING, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new SluttfagvurderingResources();
    
        ArrayList<SluttfagvurderingResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<SluttfagvurderingResource>>() {});
    ...
    public SluttfagvurderingResource getSluttfagvurderingBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, VurderingActions.GET_SLUTTFAGVURDERING, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<SluttfagvurderingResource> sluttfagvurdering = cacheService.getSluttfagvurderingBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return sluttfagvurdering.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            SluttfagvurderingResource sluttfagvurdering = objectMapper.convertValue(response.getData().get(0), SluttfagvurderingResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(sluttfagvurdering);
        }    
    }
