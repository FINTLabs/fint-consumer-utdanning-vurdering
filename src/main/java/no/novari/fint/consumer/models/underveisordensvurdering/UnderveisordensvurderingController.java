...
@Slf4j
@Api(tags = {"Underveisordensvurdering"})
@CrossOrigin
@RestController
@RequestMapping(name = "Underveisordensvurdering", value = RestEndpoints.UNDERVEISORDENSVURDERING, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class UnderveisordensvurderingController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private UnderveisordensvurderingCacheService cacheService;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Underveisordensvurdering cache is disabled.");
        }
    ...
    @GetMapping
    public UnderveisordensvurderingResources getUnderveisordensvurdering(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getUnderveisordensvurderingByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Underveisordensvurdering cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, VurderingActions.GET_ALL_UNDERVEISORDENSVURDERING, client);
        ...
        Stream<UnderveisordensvurderingResource> resources;
    ...
    @PostMapping("/$query")
    public UnderveisordensvurderingResources getUnderveisordensvurderingByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getUnderveisordensvurdering(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private UnderveisordensvurderingResources getUnderveisordensvurderingByOdataFilter(
        ...
        Event event = new Event(
                ...
                VurderingActions.GET_UNDERVEISORDENSVURDERING, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new UnderveisordensvurderingResources();
    
        ArrayList<UnderveisordensvurderingResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<UnderveisordensvurderingResource>>() {});
    ...
    public UnderveisordensvurderingResource getUnderveisordensvurderingBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, VurderingActions.GET_UNDERVEISORDENSVURDERING, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<UnderveisordensvurderingResource> underveisordensvurdering = cacheService.getUnderveisordensvurderingBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return underveisordensvurdering.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            UnderveisordensvurderingResource underveisordensvurdering = objectMapper.convertValue(response.getData().get(0), UnderveisordensvurderingResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(underveisordensvurdering);
        }    
    }
