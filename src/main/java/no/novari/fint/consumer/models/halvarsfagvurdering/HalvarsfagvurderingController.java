...
public class HalvarsfagvurderingController {
    ...
    @GetMapping
    public HalvarsfagvurderingResources getHalvarsfagvurdering(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, VurderingActions.GET_ALL_HALVARSFAGVURDERING, client);
        ...
        Stream<HalvarsfagvurderingResource> resources;
    ...
    public HalvarsfagvurderingResource getHalvarsfagvurderingBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, VurderingActions.GET_HALVARSFAGVURDERING, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<HalvarsfagvurderingResource> halvarsfagvurdering = cacheService.getHalvarsfagvurderingBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return halvarsfagvurdering.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            HalvarsfagvurderingResource halvarsfagvurdering = objectMapper.convertValue(response.getData().get(0), HalvarsfagvurderingResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(halvarsfagvurdering);
        }    
    }
